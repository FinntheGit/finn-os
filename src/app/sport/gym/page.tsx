...
export default function GymPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (!selectedExerciseId && exerciseRows.length > 0) {
        setSelectedExerciseId(exerciseRows[0].id);
      }
      ...
      setMessage(
        error instanceof Error ? error.message : "Gym Pro laden mislukt."
      );
    } finally {
      setLoading(false);
    }
  ...
  function getPR(exercise: Exercise) {
    const history = getExerciseSets(exercise);
    ...
    if (exercise.tracking_mode === "bodyweight") {
      ...
      return best > 0
        ? `${best} reps`
        : exercise.baseline_note || "Nog geen PR";
    }
    ...
    if (bestWeight === 0) {
      return exercise.baseline_note || "Nog geen PR";
    }
  ...
  function isNewPR(
    ...
    history.forEach((set) => {
      ...
      if (
        oldWeight > bestWeight ||
        (oldWeight === bestWeight && oldReps > bestReps)
      ) {
    ...
    return (
      weight > bestWeight ||
      (weight === bestWeight && (reps ?? 0) > bestReps)
    );
  }
  ...
  async function startTraining() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Training starten mislukt."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function finishTraining() {
    ...
    try {
      ...
      const exerciseCount = currentExerciseIds.size;
      ...
      setMessage(
        error instanceof Error ? error.message : "Training afronden mislukt."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function ensureExerciseSelected(exercise: Exercise) {
    ...
    const alreadySelected = sessionExercises.some(
      (item) =>
        item.session_id === activeSession.id &&
        item.exercise_id === exercise.id
    );
    ...
    await supabase.from("gym_session_exercises").insert({
      user_id: user.id,
      session_id: activeSession.id,
      exercise_id: exercise.id,
    });
  ...
  async function addSet(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const previousSetsThisSession = currentSessionSets.filter(
        (set) =>
          set.exercise_id === selectedExercise.id ||
          set.exercise_name === selectedExercise.name
      );
      ...
      await ensureExerciseSelected(selectedExercise);
      ...
      const { error } = await supabase.from("gym_sets").insert({
        ...
        set_number: previousSetsThisSession.length + 1,
        reps,
        weight_kg: weight,
      ...
      if (newPR) {
        setPrMessage(`🏆 NIEUWE PR — ${selectedExercise.name}`);
      }

      await loadData();
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Set opslaan mislukt."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function addExercise(e: FormEvent<HTMLFormElement>) {
    ...
    const trackingMode = String(
      form.get("tracking_mode") || "weight_reps"
    ) as TrackingMode;

    const weightRaw = String(form.get("baseline_weight") || "").trim();
    const repsRaw = String(form.get("baseline_reps") || "").trim();
    const durationRaw = String(form.get("baseline_duration") || "").trim();
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          tracking_mode: trackingMode,
          baseline_weight_kg: weightRaw ? Number(weightRaw) : null,
          baseline_reps: repsRaw ? Number(repsRaw) : null,
          baseline_duration_seconds: durationRaw
            ? Number(durationRaw)
            : null,
      ...
      setMessage(
        error instanceof Error ? error.message : "Oefening toevoegen mislukt."
      );
    } finally {
      setSaving(false);
    }
  ...
  const groupedExercises = useMemo(() => {
    return exercises.reduce<Record<string, Exercise[]>>(
      (groups, exercise) => {
  ...
  const previousSets = selectedExercise
    ? getPreviousSets(selectedExercise)
    : [];
    ...
    <>
      ...
      {prMessage && (
        <div className="card" style={{ marginBottom: 20 }}>
          <div className="big" style={{ fontSize: "1.7rem" }}>
            {prMessage}
          </div>
        </div>
      )}
      ...
      <section className="grid two">
        <div className="card">
          ...
          {activeSession ? (
            <>
              ...
              <div className="big">{currentExerciseIds.size}</div>

              <p className="subtle">oefeningen geselecteerd</p>

              <div className="row">
                <span>Sets</span>
                <strong>{currentSessionSets.length}</strong>
              </div>
            ...
            <>
              ...
              <p className="subtle">
                Start je sessie en tik daarna simpelweg de oefeningen aan die
                je doet.
              </p>
        ...
        <div className="card">
          ...
          <div className="list">
            <div className="row">
              <span>Geloggede sets</span>
              <strong>{sets.length}</strong>
            </div>

            <div className="row">
              <span>Gymtrainingen</span>
      ...
      {activeSession && (
        <>
          <div className="section-title">
            <div>
              <h2>Welke oefeningen doe je?</h2>
              <div className="subtle">
                Tik ze aan. Groen vinkje = onderdeel van deze training.
              </div>
            </div>
          </div>
            ...
            <form className="form" onSubmit={addSet}>
              ...
              {selectedExercise && (
                <div className="notice">
                  <strong>Huidige PR: {getPR(selectedExercise)}</strong>

                  {selectedExercise.baseline_note && (
                    <>
                      <br />
                      <span className="subtle">
                        Startwaarde: {selectedExercise.baseline_note}
                      </span>
                    </>
                  )}
                        ...
                        {previousSets
                          .map((set) => {
                            if (set.duration_seconds !== null) {
                              return formatSeconds(set.duration_seconds);
                            }
          ...
          <div className="card">
            {currentExerciseIds.size === 0 ? (
              <p className="subtle">
                Tik hierboven de oefeningen aan die je gaat doen.
              </p>
            ) : (
              <div className="list">
                {exercises
          ...
          <div className="list">
            {sessions.slice(0, 20).map((session) => {
              const sessionSetCount = sets.filter(
                (set) => set.session_id === session.id
              ).length;

              const exerciseCount = sessionExercises.filter(
                ...
                <div className="row" key={session.id}>
                  <span>
                    ...
                    <span className="subtle">
                      ...
                      {exerciseCount} oefeningen · {sessionSetCount} sets
                    </span>
                  </span>
      ...
      <div className="section-title">
        ...
        >
          {showExerciseForm ? "Annuleren" : "+ Oefening"}
        </button>
      </div>
          ...
          <form className="form" onSubmit={addExercise}>
            ...
            <div className="field">
              ...
              <select name="tracking_mode" defaultValue="weight_reps">
                <option value="weight_reps">Gewicht + reps</option>
                <option value="bodyweight">Lichaamsgewicht + reps</option>
                <option value="time">Tijd</option>
              </select>
            </div>

            <div className="field">
              <label>Huidige PR gewicht (optioneel)</label>
            ...
            <div className="field">
              <label>Huidige PR reps (optioneel)</label>

              <input name="baseline_reps" type="number" min="0" />
            </div>

            <div className="field">
              <label>Huidige PR tijd in seconden (optioneel)</label>
